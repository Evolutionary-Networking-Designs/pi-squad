 ...
 */
export function initKnowledgeDir(teamRoot) {
 ...
 */
export async function scanKnowledgeDir(teamRoot) {
    const knowledgeDir = resolveKnowledgeDir(teamRoot);
    ...
    for (const entry of entries) {
        ...
        if (entry === INGESTED_MANIFEST || entry === GITIGNORE_NAME)
            continue;
 ...
 */
export function markIngested(teamRoot, filePath) {
    const knowledgeDir = resolveKnowledgeDir(teamRoot);
    const manifestPath = join(knowledgeDir, INGESTED_MANIFEST);
 ...
 * @param teamRoot Absolute path to the `.squad/` parent directory (repo root)
 */
export function resolveKnowledgeDir(teamRoot) {
    return join(resolve(teamRoot), ".squad", KNOWLEDGE_DIR_NAME);
}
// ─── Internal helpers ─────────────────────────────────────────────────────────
function loadManifest(knowledgeDir) {
    const manifestPath = join(knowledgeDir, INGESTED_MANIFEST);
    if (!existsSync(manifestPath))
        return new Set();
    try {
...
.ingested
`;
//# sourceMappingURL=knowledge-dir.js.map