...
const KNOWN_DIRECTIVE_TYPES = new Set([
    "agent_spawn",
    "squad_update",
    "direct_response",
    "unknown",
]);
function isKnownDirectiveType(type) {
    return KNOWN_DIRECTIVE_TYPES.has(type);
}
function hasRequiredText(value) {
    return typeof value === "string" && value.trim().length > 0;
}
export function truncateForLog(s, maxLen = 80) {
...
function detectCircularSpawnReference(directive) {
...
export class GuardChecker {
    validate(directive) {
        ...
        if (directive.type === "agent_spawn") {
            ...
            if (typeof directive.systemPrompt === "string" &&
            ...
            const extraFields = Object.keys(directive).filter((field) => !AGENT_SPAWN_ALLOWED_FIELDS.has(field));
            if (extraFields.length > 0) {
                console.warn(`[pi-squad] agent_spawn contains unknown fields (ignored): ${truncateForLog(extraFields.join(", "), 200)}`);
            }
            const typedDirective = {
                type: "agent_spawn",
                agentId: directive.agentId,
                prompt: directive.prompt,
                systemPrompt: hasRequiredText(directive.systemPrompt) ? directive.systemPrompt : undefined,
                model: directive.model === "fast" ||
                ...
                spawnPath: Array.isArray(directive.spawnPath)
                    ? directive.spawnPath.filter((value) => typeof value === "string")
                    : undefined,
            };
        ...
        if (directive.type === "squad_update") {
            ...
            if (typeof directive.details === "string" &&
