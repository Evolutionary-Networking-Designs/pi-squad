...
function getSquadPath(dirPath) {
    return join(dirPath, SQUAD_DIRNAME);
}
function getTeamPath(dirPath) {
    return join(getSquadPath(dirPath), TEAM_FILENAME);
}
function hasTeamConfig(dirPath) {
    return existsSync(getTeamPath(dirPath));
}
function isGitRoot(dirPath) {
    return existsSync(join(dirPath, ".git"));
}
function parseTeamName(teamSource, dirPath) {
...
function parseAgents(teamSource) {
    const agents = [];
...
function buildTeamConfig(dirPath, teamSource) {
    return {
...
export async function buildTeamLevel(dirPath, level) {
...
export async function resolveTeamStack(cwd = process.cwd()) {
...
//# sourceMappingURL=team-stack.js.map