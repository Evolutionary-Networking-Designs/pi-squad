 ...
 */
function chunkSection(section: string, targetTokens: number): string[] {
 ...
 */
function chunkBySentences(text: string, targetTokens: number): string[] {
...
function applyOverlap(chunks: string[], overlapTokens: number): string[] {
  ...
  return chunks.map((chunk, i) => {
    ...
    if (!overlapText || chunk.startsWith(overlapText)) return chunk;
    return `${overlapText}\n\n${chunk}`;
  });
}
 ...
 */
function extractTailTokens(text: string, tokenCount: number): string {
