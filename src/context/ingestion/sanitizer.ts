 ...
 */
export function sanitize(text: string, options?: SanitizerOptions): SanitizeResult {
  ...
  let secretCount = 0;
  const countAndStrip = (re: RegExp): string =>
    out.replace(re, () => { secretCount++; return '[REDACTED]'; });
