 ...
 */
export async function syncSquadUpstream(options: SyncOptions): Promise<SyncResult> {
  ...
  try {
    ...
    const pkg = JSON.parse(pkgRaw) as {
      squad?: { minVersion?: string; maxVersion?: string };
    };
  ...
  if (!compatResult.compatible) {
    console.warn(
      `[pi-squad] sync-squad: ${compatResult.reason ?? "version out of range"} — proceeding anyway`,
    );
  }
